...
function updateRelToWF(element) {
    var line_num = element.id.match(/[0-9]+/i);
    if (!$('input[name="' + element.name + '"]').is(':checked')) {
        if ($('#aow_actions_param_parent_module' + line_num).length > 0)
            $('#aow_actions_param_parent_module' + line_num).parent().hide();
    }
    else {
        if ($('#aow_actions_param_parent_module' + line_num).length > 0) {
            $('#aow_actions_param_parent_module' + line_num).parent().show();
            if ($('#aow_actions_param_parent_module' + line_num).children().length == 0) {
                $('#aow_actions_param_parent_module' + line_num).html(options);
            }
        }
        // else if ($('#aow_actions_param_parent_module' + line_num).children().length==0){
        //     $('#aow_actions_param_parent_module' + line_num).html(options);
        // }
    }
}