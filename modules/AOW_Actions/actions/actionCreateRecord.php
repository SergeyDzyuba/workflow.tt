...
class actionCreateRecord extends actionBase
{
     ...
     * @return string
     */
    public function edit_display($line, SugarBean $bean = null, $params = array(), $parent_process_id = '')
    {
        global $app_list_strings, $db;
        if (isset($_REQUEST['id']) && !empty($_REQUEST['id'])) {
            $action = BeanFactory::getBean('AOW_Actions', $_REQUEST['id']);
        }
...
*</span>&nbsp;&nbsp;';
        ...
        $html .= "<input type='checkbox' id='aow_actions_param[" . $line . "][relate_to_workflow]' name='aow_actions_param[" . $line . "][relate_to_workflow]' value='1' $checked    onchange='updateRelToWF(this)'></td>";
        $html .= '<td id="name_label" class="name_label" scope="row" valign="top" '.$hidden.'><label>' .
...
*</span>&nbsp;&nbsp;';
        ...
        if (isset($params['record_type']) && $params['record_type'] != '') {
            ...
            if (isset($params['parent_module']) && !empty($params['parent_module'])){
                $html .= 'cr_selected_parent_module[' . $line . '] = "' . trim($params['parent_module']) . '";';
            }
            $html .= '</script>';
        }
        return $html;
    ...
    /**
     * @param SugarBean $bean запись, на которую реакция
     * @param array $params
     * @param bool $in_save
     * @return bool
     *
     * $record - создаваемая запись
     */
    public function run_action(SugarBean $bean, $params = array(), $in_save = false, $is_subprocess = false, $parent_record_module = '', $parent_record_id = '')
    {
        global $beanList, $db;
        ...
        if (isset($params['record_type']) && $params['record_type'] != '') {
            if ($beanList[$params['record_type']]) {
                ...
                $created_record_id = $this->set_record($record, $bean, $params);
                $this->set_relationships($record, $bean, $params);
                if (isset($params['relate_to_workflow']) && $params['relate_to_workflow']) {
                    require_once 'modules/Relationships/Relationship.php';
                    if ($is_subprocess && !empty($parent_record_module) && !empty($parent_record_id)) {
                        $bean = BeanFactory::getBean($parent_record_module, $parent_record_id);
                    }
